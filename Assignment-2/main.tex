...
\end{abstract}
\begin{flushleft}
\textbf{Problem 16(b): } If $\overrightarrow{a}$ and $\overrightarrow{b}$ are non-collinear vectors, find the value of $x$ such that vectors $\overrightarrow{\alpha} = \brak{x-2}\overrightarrow{a} + \overrightarrow{b}$ and $\overrightarrow{\beta} = \brak{3 + 2x}\overrightarrow{a} - 2\overrightarrow{b}$ are collinear.
